...
- `MEMENTO_ACCESS_KEY`를 설정하지 않으면 서버가 기동하지 않고 종료 코드 78로 멈춘다. 인증 없이 운용하려면 `MEMENTO_AUTH_DISABLED=true`를 함께 지정해야 한다. 종전에는 키가 없으면 모든 요청이 master 권한으로 통과했다.
- in-process 교차 인코더 리랭커가 기본 비활성으로 바뀌었다. 기본 모델이 영어 전용이라 한국어 코퍼스에서 순손실이었다. 켜려면 `MEMENTO_RERANKER_ENABLED=true`를 지정한다. `RERANKER_URL`로 붙인 외부 리랭커는 영향을 받지 않는다.
- 요청 수신 상한이 60초, 사용자 요청 경로의 질의 시간 상한이 30초로 기본 설정된다. 종전에는 둘 다 무제한이었다. `REQUEST_TIMEOUT_MS`와 `DB_STATEMENT_TIMEOUT_MS`로 조정한다.
- `migration-045`가 `fragments`와 `fragment_links`에 행 수준 보안을 켜고 정책을 건다. 애플리케이션이 표 소유자인 배포에서는 동작이 바뀌지 않는다. 실제 방어선으로 세우는 절차는 `docs/operations/row-level-security.md`에 있다.
- 개발용 단위시험은 Node.js 24를 요구한다. 서버 실행은 종전대로 Node.js 20 이상에서 동작한다.

### Security

- 인증 키 미설정 시 모든 요청을 master 권한으로 통과시키던 결함을 제거했다. 키 부재 처리를 단일 정책 함수로 모으고, 구성이 어긋나면 기동 자체를 중단한다.
- workspace 허가 조회 실패를 무제한 허용과 구분한다. 종전에는 조회에 실패하면 제한 없음으로 처리해 데이터베이스가 흔들릴 때마다 격리가 풀렸다.
- 참조되지 않는 `@modelcontextprotocol/sdk` 의존성을 제거해 전이 취약점을 없앴다.
- 갱신 확인이 다른 프로젝트의 npm 패키지를 가리키던 문제를 고쳤다.

### Added
- `fragments`와 `fragment_links` 행 수준 보안 정책. `app.current_key_id` 세션 변수로 키 축 격리를 표현한다.
- 취약점 신고 절차(`SECURITY.md`), CodeQL 정적 분석, Dependabot 의존성 갱신, CI 의존성 감사 단계.
- 통합시험이 CI에서 실행된다. 외부 서비스 없이 도는 것을 전제로 작성돼 있으므로 실제로 그런지 확인한다.
- Node 20·22·24 런타임 기동 검증 행렬.

### Changed
- in-process 리랭커를 기본 비활성으로 바꿨다. 절제 실험 결과 격리 모드 Recall@5가 67퍼센트에서 85퍼센트로, 코퍼스 모드 Recall@1이 74퍼센트에서 85퍼센트로 올랐고 p50 지연은 561ms에서 126ms로 줄었다. 원인은 기본 모델이 영어 MS MARCO 전용이라 한국어 질의에서 상류 벡터 순위를 다시 쓰며 손해를 낸 것이다. 상위 30건 중 15건만 남기는 절단이라 하류 보정으로 복구되지 않는다.
- 배포물을 런타임 필수 자산으로 한정한다. 8.37MB에서 1.82MB로 줄었다.
- 빈 데이터베이스에서 `npm run migrate` 한 번으로 기반 스키마와 마이그레이션이 모두 적용된다. 종전에는 첫 마이그레이션이 기존 표를 전제해 즉시 실패했다.
- 미사용 변수 경고를 정리하고 규칙을 실패로 승격했다.

### 구조 정리

기능과 응답 표면은 바꾸지 않았다. 회귀 판정은 오프라인 계측으로 했다.

- 키 격리 조건을 SQL로 직접 쓰던 지점을 이름 붙은 생성기 호출로 모았다. 빈 배열을 격리 해제로 다루던 위험을 함께 제거했다.
- 워커 네 종의 폴링·백오프·종료 배수를 공통 기반으로 옮기고 레지스트리로 등록한다. 워커를 추가할 때 종료 경로를 함께 고쳐야 하는 결합이 사라졌다.
- 500줄이 넘던 함수 다섯을 분해했다. 최대 함수 길이가 96줄이다.
- 스키마 이름 선언 36곳과 할당량 회계 질의 6곳을 각각 한곳으로 모았다.
- 설정 모듈 참조 경로를 정리했다.
- 도구 정의, 응답 키 집합, 관리 경로, CLI 표면을 스냅샷으로 고정해 표면 변경이 시험으로 잡히게 했다.

### 의존성

- `openai` 7, `dotenv` 17, `ioredis` 6, `@huggingface/transformers` 4, `eslint` 10으로 상향했다.
- `@huggingface/transformers` 4가 구 `onnxruntime-node`를 통해 끌어오던 `adm-zip` 취약점을 override로 차단했다.
- 참조되지 않는 `@modelcontextprotocol/sdk`를 제거했다.

### Fixed
- 단위시험 간헐 실패 두 건의 원인을 제거했다. 응답 형태 스냅샷이 확률 표집을 막지 않았고, reflect 시험이 fire-and-forget 작업을 배수하지 않았다.
- 벤치마크 하네스가 적재 전에 같은 격리 스코프의 잔여 파편을 회수한다. 실행이 중간에 종료되면 정리 블록이 실행되지 않아 적재분이 남고, 그 상태로 다음 실행이 적재하면 이전 파편이 방해 요소로 섞여 측정이 오염됐다.
- 벤치마크 정리 단계가 삭제 건수를 항상 0으로 보고하던 문제 수정. `deleteByAgent`가 rowCount 숫자를 반환하는데 객체로 다뤘다.
