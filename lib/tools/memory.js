...
import { findRecordedResponse, recordResponse } from "../memory/write/IdempotencyStore.js";
...
/**
 * `_sessionId`(전송계층 세션 ID)를 제거하고 유휴·수명 기준으로 회전하는 파생
 * 세그먼트 ID(`currentSegmentId`)를 계산한다. 클라이언트가 sessionId를 직접
 * 전달한 경우에는 args.sessionId를 덮어쓰지 않는다(세그먼트는 `_sessionId`
 * 폴백 경로에만 적용).
 *
 * @param {Object} args - in-place mutation (args.sessionId 는 폴백 시에만 갱신)
 * @returns {Promise<string|undefined>} 세션 활동 추적에 쓸 파생 세그먼트 ID
 */
async function resolveSegmentedSessionId(args) {
  const rawSessionId = args._sessionId;
  delete args._sessionId;

  if (!rawSessionId) return rawSessionId;

  const segmentId = await currentSegmentId(rawSessionId);
...
export async function tool_recall(args) {
  ...
  try {
    ...
    if (args.includeContext) {
      ...
      /** 시간 인접 파편과 인과 링크를 하나의 서사 구조로 조합한다.
       *
       *  대상은 순위 상위 N건이 아니라 "조합할 재료가 실제로 있는" 상위 N건이다.
       *  인과 링크와 세션 인접 파편은 드물게 존재하므로, 순위만으로 자르면 재료가
      ...
      const [causalMap, neighborMap] = await Promise.all([
        ...
        fetchSessionNeighbors(result.fragments, stitchScope).catch((err) => {
          logWarn("fetchSessionNeighbors failed", { error: err.message });
          return new Map();
        }),
      ]);
      ...
      for (const frag of result.fragments) {
        if (stitches.length >= STITCH_MAX_TARGETS) break;

        const nearby = neighborMap.get(frag.id) || frag.nearby_context || [];
        const linked = causalMap.get(frag.id) || [];
        const stitch = stitchFragment(frag, { nearby, linked });
        if (stitch) stitches.push({ fragmentId: frag.id, stitch });
      }

      const kept   = applyStitchBudget(stitches, args.tokenBudget || 1000);
      const keptBy = new Map(kept.map(k => [k.fragmentId, k.stitch]));

      for (const frag of result.fragments) {
        const stitch = keptBy.get(frag.id);
        if (stitch) frag.stitched_context = stitch;
      }
    }
...
/**
 * tool_recall args를 정규화한다: _sessionId를 파생 세그먼트 ID로 승격시키고
 * asOf를 anchorTime(epoch ms)으로 변환한다.
 *
 * @param {Object} args - tool_recall args (in-place mutation)
 * @returns {Promise<string|undefined>} sessionId
 * @throws {Error} asOf가 유효하지 않은 날짜 문자열일 때
 */
async function normalizeRecallParams(args) {
  const sessionId = await resolveSegmentedSessionId(args);
 ...
 */
async function hasPendingContradictions(fragmentIds, scope = {}) {
...
function buildRecallResponse(result, { args, linkedMap, contradictionPending = false }) {
  ...
  if (result.caseMode) {
    /** 케이스가 반환됐는데 fragments 배열이 없다는 이유로 no_results를 내지 않는다. */
    const hint = result.caseCount === 0 ? buildRecallHint([], args) : null;
  ...
  const fragments = result.fragments.map(f => {
    const projected = {
      ...
      /** 시간 인접 파편과 인과 링크를 하나의 서사로 묶은 구조. includeContext=true에서만 생성된다. */
      ...(f.stitched_context          ? { stitched_context: f.stitched_context } : {}),
 ...
 * 재시도 안전 실행기.
 *
 * `idempotencyKey`가 주어지면 첫 호출의 응답을 기록하고, 같은 키의 재호출에는
 * 본래 작업을 다시 수행하지 않고 그 응답을 그대로 돌려준다. 네트워크가 끊겨
 * 응답을 못 받은 클라이언트가 다시 부를 때 이력이 두 번 쌓이는 것을 막는다.
 *
 * 키가 없으면 아무것도 하지 않고 그대로 실행한다. 기존 호출 방식은 그대로다.
 *
 * @param {string}   tool
 * @param {Object}   args
 * @param {Function} run  실제 작업. 응답 객체를 반환한다.
 * @returns {Promise<Object>}
 */
async function withIdempotency(tool, args, run) {
  const key = args?.idempotencyKey;
  if (!key) return run();

  const keyId    = args._keyId ?? null;
  const agentId  = args.agentId ?? "default";
  const recorded = await findRecordedResponse(tool, key, keyId, agentId);
  if (recorded) return { ...recorded, idempotentReplay: true };

  const response = await run();
  await recordResponse(tool, key, response, keyId, agentId);
  return response;
}
...
export async function tool_reflect(args) {
  const mgr = MemoryManager.getInstance();
  return withIdempotency("reflect", args, () => withAudit("reflect", args, async (a, sessionId) => {
...
export async function tool_toolFeedback(args) {
  delete args._sessionId;
  const mgr = MemoryManager.getInstance();
  return withIdempotency("tool_feedback", args, async () => {
  try {
...
function buildRecallHint(fragments, args, flags = {}, topicCandidates = []) {
  if (!fragments.length && Array.isArray(topicCandidates) && topicCandidates.length > 0) {
    ...
    const workspaceNote = resolveWorkspaceScope(args).mode === "global_only"
      ? " 현재 조회는 전역(workspace 없음) 범위만 대상으로 합니다. workspace별 기억을 찾으려면 해당 workspace를 지정해 다시 recall 하세요."
      : "";
    return {
      signal    : "topic_mismatch",
      suggestion: `topic '${args?.topic ?? ""}' 파편이 없습니다. 유사 토픽: ${listed}. topic=${topicCandidates[0].topic}으로 재검색하세요.${workspaceNote}`,
      trigger   : "recall"
    };
  }
  if (!fragments.length && resolveWorkspaceScope(args).mode === "global_only") {
    return {
      signal    : "no_results",
      suggestion: "전역(workspace 없음) 범위에서만 검색했으며 결과가 없습니다. workspace별로 저장한 기억을 찾으려면 해당 workspace를 지정해 다시 recall 하세요. 전체 workspace 조회는 master의 관리 목적 호출에서만 allWorkspaces=true로 요청할 수 있습니다.",
      trigger   : "recall"
    };
  }
  if (!fragments.length) {
