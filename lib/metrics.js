...
/** 자동 앵커 승격 설정 상태 (1=enabled, 0=disabled). */
export const anchorAutoPromotionEnabled = new prometheus.Gauge({
  name     : "mcp_anchor_auto_promotion_enabled",
  help     : "Whether automatic anchor promotion is enabled by configuration",
  registers: [register]
});

/** 서버 기동 시 자동 앵커 승격 설정을 Gauge에 반영한다. */
export function setAnchorAutoPromotionEnabled(enabled) {
  anchorAutoPromotionEnabled.set(enabled === true ? 1 : 0);
}
