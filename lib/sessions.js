 ...
 * @param {string|null} keyId  DB API 키 ID (마스터 키 세션은 null)
 */
export async function createStreamableSession(authenticated = false, keyId = null, groupKeyIds = null, permissions = null, defaultWorkspace = null, mode = null, isMaster = false) {
  const sessionId           = crypto.randomUUID();
  return createStreamableSessionWithId(
    sessionId, authenticated, keyId, groupKeyIds, permissions,
    defaultWorkspace, mode, null, isMaster
  );
}

/**
 * 세션 레코드의 세그먼트 상태를 현재 시각 기준으로 갱신한다 (in-place mutation).
 * 유휴(idleMs) 초과 또는 세그먼트 절대 수명(maxAgeMs) 초과 시 segmentSeq를 증가시킨다.
 *
 * @param {Object} session
 * @param {number} now
 * @returns {number} 직전 segmentSeq (회전이 일어났으면), 아니면 null
 */
function advanceSegment(session, now) {
  const { idleMs, maxAgeMs } = MEMORY_CONFIG.sessionSegment;
...
/**
 * 세션의 현재(회전 반영 후) 파생 세그먼트 ID의 segmentSeq만 읽어 조합한다.
 * 회전 판정 없이 순수 조합만 수행 — close/idle-reflect 등 "지금 이 순간의
 * 세그먼트를 대상으로 삼되 더 회전시키지는 않는" 호출부용.
 *
 * @param {Object} session
 * @param {string} sessionId
 * @returns {string}
 */
function reflectTargetId(session, sessionId) {
  if (!MEMORY_CONFIG.sessionSegment.enabled) return sessionId;
  const seq = session?.segmentSeq ?? 1;
  return `${sessionId}#${seq}`;
}

/**
 * 세션 레코드를 메모리(streamableSessions) 우선, 없으면 Redis에서 조회한다.
 * 세그먼트 조회 전용 — TTL 갱신 등 부작용 없는 순수 조회.
 *
 * @param {string} sessionId
...
async function resolveSessionRecord(sessionId) {
  ...
  if (!session && REDIS_ENABLED) {
    ...
    if (redisSession) session = redisSession;
  }
...
/**
 * 세션 레코드를 Redis에 영속화한다 (비직렬화 필드 제외).
 *
 * @param {string} sessionId
 * @param {Object} session
 */
async function persistSegmentState(sessionId, session) {
  ...
  const remainingTtlSec = Math.max(
    Math.ceil(((session.expiresAt ?? Date.now() + SESSION_TTL_MS) - Date.now()) / 1000),
    60
  );
...
/**
 * 전송계층 세션 ID로부터 현재 파생 논리 세션(세그먼트) ID를 계산한다.
 *
 * 도구 호출 시점마다 호출되어 유휴·수명 기준 회전 여부를 판정하고, 회전이
 * 일어나면 직전 세그먼트에 대해 autoReflect를 fire-and-forget으로 실행한다.
 * sessionSegment.enabled=false이거나 세션을 찾을 수 없으면 원본 ID를 그대로
 * 반환한다(현행 동작 유지).
 *
 * @param {string} sessionId 전송계층(MCP-Session-Id) 세션 ID
 * @returns {Promise<string>} 파생 세그먼트 ID (`${sessionId}#${seq}`) 또는 원본 ID
 */
export async function currentSegmentId(sessionId) {
  ...
  const now          = Date.now();
  const previousSeq   = advanceSegment(session, now);

  if (previousSeq != null) {
    autoReflect(`${sessionId}#${previousSeq}`).catch(() => {});
  }

  await persistSegmentState(sessionId, session);

  return `${sessionId}#${session.segmentSeq}`;
}

/**
 * 세션의 "현재" 파생 세그먼트 ID를 회전 없이 조회 전용으로 반환한다.
 * 관리자 도구 등 외부에서 특정 전송계층 세션의 최신 세그먼트를 대상으로
 * autoReflect를 재실행하거나 표시할 때 사용한다.
 *
 * @param {string} sessionId 전송계층 세션 ID
 * @returns {Promise<string>} 파생 세그먼트 ID 또는 원본 ID
 */
export async function peekCurrentSegmentId(sessionId) {
  if (!sessionId) return sessionId;
  if (!MEMORY_CONFIG.sessionSegment.enabled) return sessionId;

  const session = await resolveSessionRecord(sessionId);

  return reflectTargetId(session, sessionId);
}
...
export async function closeStreamableSession(sessionId, { preserveRedis = false } = {}) {
  ...
  /** 세션 종료 전 자동 reflect (비차단: 실패해도 세션은 닫힘) — 현재 세그먼트 대상 */
  try { await autoReflect(reflectTargetId(session, sessionId)); } catch { /* noop */ }
 ...
 */
export async function closeLegacySseSession(sessionId, { _preserveRedis = false } = {}) {
  const session             = legacySseSessions.get(sessionId);
  ...
  for (const [sessionId, session] of streamableSessions.entries()) {
    if (now > session.expiresAt) {
      ...
      if (isIdleEnough && needsReflect) {
        try {
          await autoReflect(reflectTargetId(session, sessionId));
